...
uniform float NGSS_PCSS_FILTER_LOCAL;
#define NGSS_PCSS_FILTER_SPOT_MIN 0.025 							//Close to blocker (If 0.0 == Hard Shadows). This value cannot be higher than NGSS_PCSS_FILTER_POINT_MAX
#define NGSS_PCSS_FILTER_SPOT_MAX 1.0 								//Far from blocker (If 1.0 == Soft Shadows). This value cannot be smaller than NGSS_PCSS_FILTER_POINT_MIN
...
{
	#if defined(SHADOWS_SOFT)//#if defined(NGSS_SHADOWS_DITHERING)
	/*//2D arrays creates compile errors, plus is not well supported in some platforms
	const static float ditherPatternLocal[4][4] = {
		{ 0.0, 0.5, 0.125, 0.625},
	...
	*/	
    ...
    //return lerp(0, frac(sin(dot(magic, magic)) * 43758.5453f), NGSS_BANDING_TO_NOISE_RATIO) * UNITY_TWO_PI;
	float2 magic = float2(
        23.14069263277926, // e^pi (Gelfond's constant)
         2.665144142690225 // 2^sqrt(2) (Gelfondâ€“Schneider constant)
    );
    return lerp(0, frac(cos(dot(position_screen, magic)) * 12345.6789), NGSS_BANDING_TO_NOISE_RATIO);
	#endif
}
...
// ------------------------------------------------------------------
	...
	inline fixed UnitySampleShadowmapNGSS(float4 shadowCoord, float2 screenPos)
	{
		...
		#if defined(SHADOWS_SOFT)//#if defined(NGSS_SHADOWS_DITHERING)
		float randPied = InterleavedGradientNoise(screenPos) * UNITY_TWO_PI;
		#else
		float2 spos = tex2D(unity_RandomRotation16, screenPos.xy).xy;//coord.xy / 8?		
		float randPied = InterleavedGradientNoise(spos) * UNITY_TWO_PI;//6.28318530718f
		#endif
		...
		if(NGSS_TEST_SAMPLERS > 0)
		{
				...
				float2 distances = BLOCKER_SEARCH_SPOT(coord, diskRadiusPCSS, randPied, NGSS_TEST_SAMPLERS, dz_duv);
				...
				diskRadiusPCSS *= (coordz - distances.x)/(distances.x);
				#else
				diskRadiusPCSS *= (coord.z - distances.x)/(distances.x);
				#endif
				
				diskRadiusFinal = lerp(diskRadiusPCF, diskRadiusPCSS, NGSS_PCSS_FILTER_LOCAL);//Lerp between PCF and PCSS
			#else// NO NGSS_CAN_USE_PCSS_FILTER
				
				half shadowTest = PCF_FILTER_SPOT(coord, diskRadiusFinal, randPied, NGSS_TEST_SAMPLERS, dz_duv);
...
// ------------------------------------------------------------------
	...
	inline half UnitySampleShadowmapNGSS(float3 vec, float2 screenPos) //screenPos the same pos as when fetching screen space shadow mask
	{
		...
		#else
		float2 screenPos2 = tex2D(unity_RandomRotation16, screenPos.xy).xyz;// spos.xy
		float randPied = InterleavedGradientNoise(screenPos2.xy) * UNITY_TWO_PI;//6.28318530718f
		#endif
