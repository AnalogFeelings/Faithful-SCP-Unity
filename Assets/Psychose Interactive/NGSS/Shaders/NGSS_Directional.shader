...
{
	#if defined(NGSS_NOISE_GRID_DIR)
	/*
	const static float ditherPatternDir[4][4] = {
		{ 0.0f, 0.5f, 0.125f, 0.625f},
		{ 0.75f, 0.22f, 0.875f, 0.375f},
		{ 0.1875f, 0.6875f, 0.0625f, 0.5625},
		{ 0.9375f, 0.4375f, 0.8125f, 0.3125}};
	*/
	float ditherValue = ditherPatternDir[position_screen.x * NGSS_BANDING_TO_NOISE_RATIO_DIR * _ScreenParams.x % 4][position_screen.y * NGSS_BANDING_TO_NOISE_RATIO_DIR * _ScreenParams.y % 4] * UNITY_PI * 0.75;
	...
	//position_screen /= _BlueNoiseTexture_TexelSize.xy; // the dither mask texture is 16x16
	//Blue Noise, requires a precomputed noise texture.
	//return lerp(0, tex2D(_BlueNoiseTexture, position_screen).r, NGSS_BANDING_TO_NOISE_RATIO_DIR);
	return ditherValue;//lerp(0, ditherValue, NGSS_BANDING_TO_NOISE_RATIO_DIR);
	#else
...
//PCSS FORMULA: penumbraSize = (receiver - avBlocker) * lightSize / receiver;
	...
	diskRadius = shadowSoftness * 0.0625;
	#else
...
Subshader
{
	...
	Pass
	{
		...
		#pragma shader_feature NGSS_NOISE_GRID_DIR
