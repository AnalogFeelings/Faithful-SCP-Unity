	...
	void OnRenderObject()
	{
        ...
        */
        ...
        RenderTargetIdentifier[] mrt = { BuiltinRenderTextureType.GBuffer0, BuiltinRenderTextureType.GBuffer1,
            BuiltinRenderTextureType.GBuffer2, (cam.allowHDR ? BuiltinRenderTextureType.CameraTarget : BuiltinRenderTextureType.GBuffer3) };
        buf.SetRenderTarget(mrt, BuiltinRenderTextureType.CameraTarget);
        for (int i = 0; i < DecalSystem.instance.avaiDecals; i++ )
        {
            buf.DrawMesh(m_CubeMesh, Matrix4x4.TRS(DecalSystem.instance.DecalPool[i].position,
                Quaternion.Euler(DecalSystem.instance.DecalPool[i].rotation),
                new Vector3(DecalSystem.instance.DecalPool[i].Scale, 0.25f, DecalSystem.instance.DecalPool[i].Scale)), DecalSystem.instance.DecalAtlas);
        }
